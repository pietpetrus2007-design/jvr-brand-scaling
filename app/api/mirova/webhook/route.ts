...
async function grantWAConsent(phone: string, email: string | null) {
  ...
  const profileAttrs: Record<string, any> = {
    ...
    subscriptions: {
      whatsapp: {
        marketing: { consent: 'SUBSCRIBED', consented_at: new Date(Date.now() - 300000).toISOString() },
        transactional: { consent: 'SUBSCRIBED', consented_at: new Date(Date.now() - 300000).toISOString() }
      }
    }
  }
