Add a "Questions about this lesson?" Q&A section at the bottom of every lesson in the course view.

## How it works
- Each lesson has a Q&A section below the slides and resources
- Student types a question → AI answers in context of that specific lesson
- The AI knows which lesson/module the student is on
- Conversation is cleared when switching to a different lesson

## 1. Update /api/ai/chat route to accept lessonContext

The route already accepts `lessonContext` in the body. Update the system prompt to use it:

When lessonContext is provided, prepend to the user message:
"[LESSON CONTEXT: The student is viewing: {lessonContext}]\n\n{question}"

The system prompt already covers this - just make sure lessonContext is used.

## 2. Create LessonQA component

Create app/dashboard/LessonQA.tsx:
...
## 3. Add LessonQA to CourseView.tsx

Import LessonQA and add it at the bottom of the selected lesson view, after resources:

```tsx
import LessonQA from "@/app/dashboard/LessonQA"

// Inside the lesson content area, after resources:
...
Make sure selectedModule is tracked in state alongside selectedLesson.

## After building:
- npm run build (fix errors)
- git add -A && git commit -m "feat: lesson-specific AI Q&A in every lesson"
- git push origin main
- vercel --prod
- openclaw system event --text "Done: lesson Q&A deployed" --mode now
